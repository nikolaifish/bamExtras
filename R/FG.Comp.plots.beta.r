...
#  Revised by R. Cheshire, January 2019 (add boxplots, weighting of pooled plots, plotting
#                                         pooled comps by selectivity blocks)
#######################################################################################
...
{  Errstring = ("No composition data found.  Terminating Comp.plots");
   ...
   for (iplot in 1:nplots)
   {
     ...
     #start operation for matrix
     if(is.matrix(m1.all)==TRUE){
     sel.block <- sel[[paste("sel.m",FGTrimName(names(cm)[iplot * 2], removePrefix = 1, removeSuffix = 1),sep=".")]]
     sel.block <- sel.block[rownames(sel.block)%in%rownames(m1.all),]
     if(!is.matrix(sel.block))warning(paste("No matching selectivity block for",names(cm)[iplot*2],"\n",sep=" "))
     block.text <- rownames(unique(as.data.frame(sel.block)))
     if(is.matrix(sel.block)){
     yr.block <- as.numeric(rownames(unique(as.data.frame(sel.block))))
     #add last year to v`ector of starting years do define intervals for blocks
     ...
     for(iyrs in 1:(length(yr.block)-1)){
       ...
       m1.sub<-m1.all[rownames(m1.all)%in%yr.block[iyrs]:(yr.block[iyrs+1]-1),]
       m2.sub<-m2.all[rownames(m2.all)%in%yr.block[iyrs]:(yr.block[iyrs+1]-1),]
       } else {
         m1.sub<-m1.all[rownames(m1.all)%in%yr.block[iyrs]:(yr.block[iyrs+1]),]
         m2.sub<-m2.all[rownames(m2.all)%in%yr.block[iyrs]:(yr.block[iyrs+1]),]
       }
     ...
     legend(x="topright",bg="white",legend=c(yr.block[iyrs]))
       ...
       neffname<-paste(gfileroot,".neff", sep="")
       nname<-paste(gfileroot,".n", sep="")
       if (neffname%in%names(x$t.series)&dim(m1.all)[1]>1) {
         nseries<-x$t.series[,names(x$t.series)==neffname]
         yrs.include<-x$t.series$year[nseries>0]
         ...
         wt=x$t.series[[neffname]][x$t.series[[neffname]]>0&x$t.series$year%in%as.numeric(rownames(m1))]
         wt=wt[is.na(wt)==FALSE]
       } else if(nname%in%names(x$t.series)&dim(m1.all)[1]>1) {
         nseries<-x$t.series[,names(x$t.series)==nname]
         yrs.include<-x$t.series$year[nseries>0]
         m1<-m1.all[rownames(m1.all)%in%yrs.include,]
         m2<-m2.all[rownames(m2.all)%in%yrs.include,]
       ...
       plot(x=x.vec,y=ob.m,col=NULL,ylim=c(0,ymax),xlab=title.y,ylab=paste("Pooled",poolylabtxt,sep="-"),main=titleroot)
       #polygon(x=c(min(x.vec),x.vec,max(x.vec)),y=c(0,ob.m,0),border="lightgray",col="lightgray")
       grid(col = "lightgray", lty = 1)
       points(x=x.vec,y=ob.m,col=col.obsd,pch=21, cex = 1.25, lwd=2)
       lines(x=x.vec,y=pr.m,col=Y1Col,lwd=2,type="l"); points(x=x.vec,y=pr.m,col=Y1Col,lwd=2, pch=16)
   ...
   {  #split.screen(smatrix)
	...
	if(is.matrix(m1)==TRUE){
      ...
      m12dp <- sum(m1*m2)   # This returns a vector of length nyear
      m11dp <- sum(m1*m1)   # Ditto
      m22dp <- sum(m2*m2)   # Ditto
      }
