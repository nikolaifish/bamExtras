...
F_calc <-  function(ac, h, R0, sigma=0, M, Fmax=1, F_n=101, sel, PS, W, ep, Px=.40,
...
# FOR EACH LEVEL OF F..
  ...
  # NON-EQUILIBRIUM CALCULATIONS
    # ALL FISH
      N_a <- expDecay(age=ac,Z=M+sel*F[fi],N0=1,plus_group=plus_group)

    # Numbers at age in the unfished population (when F=0)
    if(F[fi]==0){
      D_N_a_F0 <- data.frame("age_class"=ac, "N_F0"=N_a)
    }
...
data <- data.frame("F"=F, "spr"=spr, "SPR"=SPR,
  ...
  if(plots){
    ...
    plotNK(F,SPR,  main="Non-Equilibrium", rp=SPRMSY)
      ...
      text(F_Px,SPR_Px, labels=bquote(SPR[.(Px)] == .(signif(SPR_Px,plot_digits))),pos=4)
...
invisible(list("D_N_a_F0"=D_N_a_F0, "RefPts"=ref_points, "Data"=data))
}
