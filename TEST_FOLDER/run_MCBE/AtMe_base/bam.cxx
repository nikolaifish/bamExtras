...
open_r_info_list("parms", false);
	...
	wrt_r_item("q.nad", mfexp(log_q_cpue_nad));
        wrt_r_item("q.mad", mfexp(log_q_cpue_mad));
        wrt_r_item("q.sad", mfexp(log_q_cpue_sad));
	wrt_r_item("q.jai", mfexp(log_q_cpue_jai));
        wrt_r_item("q2.jai", mfexp(log_q2_jai));
        wrt_r_item("q.mar", mfexp(log_q_cpue_mar));
        wrt_r_item("q.eco", mfexp(log_q_cpue_eco));
...
open_r_df("parm.cons",1,8,2);
    ...
    wrt_r_df_col("log_dm_lenc_nad",log_dm_nad_lc_out);
    wrt_r_df_col("log_dm_lenc_mad",log_dm_mad_lc_out);
    //wrt_r_df_col("log_dm_sad_lc",log_dm_sad_lc_out);   
    wrt_r_df_col("log_dm_agec_cRn",log_dm_cRn_ac_out);
    wrt_r_df_col("log_dm_agec_cRs",log_dm_cRs_ac_out);
    wrt_r_df_col("log_dm_agec_cBn",log_dm_cBn_ac_out);
    wrt_r_df_col("log_dm_agec_cBs",log_dm_cBs_ac_out);
    ...
    wrt_r_df_col("log_q_cpue_nad",log_q_nad_out);
    wrt_r_df_col("log_q_cpue_mad",log_q_mad_out);
    wrt_r_df_col("log_q_cpue_sad",log_q_sad_out);
    wrt_r_df_col("log_q_cpue_jai",log_q_jai_out);
    wrt_r_df_col("log_q2_jai",log_q2_jai_out);
    wrt_r_df_col("log_q_cpue_mar",log_q_mar_out);
    wrt_r_df_col("log_q_cpue_eco",log_q_eco_out);
    
    wrt_r_df_col("log_avg_F_L_cRn",log_avg_F_cRn_out);
    wrt_r_df_col("log_avg_F_L_cRs",log_avg_F_cRs_out);
    wrt_r_df_col("log_avg_F_L_cBn",log_avg_F_cBn_out);
    wrt_r_df_col("log_avg_F_L_cBs",log_avg_F_cBs_out);
...
open_r_df("parm.tvec.cons",1,3,2);
        ...
        wrt_r_df_col("log.F.dev.cRn",set_log_dev_F_L_cRn);	
        wrt_r_df_col("log.F.dev.cRs",set_log_dev_F_L_cRs);
        wrt_r_df_col("log.F.dev.cBn",set_log_dev_F_L_cBn);
        wrt_r_df_col("log.F.dev.cBs",set_log_dev_F_L_cBs);
	wrt_r_df_col("log.q.dev.nad.RWq", set_log_dev_RWq);
	wrt_r_df_col("log.q.dev.mad.RWq", set_log_dev_RWq);
	wrt_r_df_col("log.q.dev.sad.RWq", set_log_dev_RWq);
	wrt_r_df_col("log.q.dev.jai.RWq", set_log_dev_RWq);
close_r_df();
...
open_r_vector("like");
    ...
    wrt_r_item("w.U.nad", w_cpue_nad);
    wrt_r_item("w.U.mad", w_cpue_mad);
    wrt_r_item("w.U.sad", w_cpue_sad);
    wrt_r_item("w.U.jai", w_cpue_jai);
    wrt_r_item("w.U.mareco", w_cpue_mareco);
     	
    wrt_r_item("w.lc.nad", w_lenc_nad);
    wrt_r_item("w.lc.mad", w_lenc_mad);
    //wrt_r_item("w.lc.sad", w_lc_sad);
    	
    wrt_r_item("w.ac.cRn", w_agec_cRn);
    wrt_r_item("w.ac.cRs", w_agec_cRs);
    wrt_r_item("w.ac.cBn", w_agec_cBn);
    wrt_r_item("w.ac.cBs", w_agec_cBs);
...
open_r_list("sel.age");
    ...
    wrt_r_namevector(styr_cpue_nad,endyr_cpue_nad);
    wrt_r_namevector(agebins);
    close_r_matrix();
	
    open_r_matrix("sel.m.mad");
    wrt_r_matrix(sel_mad, 2, 2);
    wrt_r_namevector(styr_cpue_mad,endyr_cpue_mad);
    wrt_r_namevector(agebins);
    close_r_matrix();

    //open_r_matrix("sel.m.sad");
    //wrt_r_matrix(sel_sad, 2, 2);
    //wrt_r_namevector(styr_cpue_sad,endyr_cpue_sad);
    //wrt_r_namevector(agebins);
    //close_r_matrix();
...
open_r_list("comp.mats");
    ...
    wrt_r_matrix(obs_agec_cRn, 2, 2);
    wrt_r_namevector(yrs_agec_cRn);
    ...
    wrt_r_matrix(obs_agec_cRs, 2, 2);
    wrt_r_namevector(yrs_agec_cRs);
    wrt_r_namevector(agebins_agec);
    close_r_matrix();
    ...
    wrt_r_matrix(obs_agec_cBn, 2, 2);
    wrt_r_namevector(yrs_agec_cBn);
    wrt_r_namevector(agebins_agec);
    close_r_matrix();
    ...
    wrt_r_matrix(obs_agec_cBs, 2, 2);
    wrt_r_namevector(yrs_agec_cBs);
    wrt_r_namevector(agebins_agec);
    close_r_matrix();
    ...
    wrt_r_matrix(obs_lenc_nad, 2, 2);
    wrt_r_namevector(yrs_lenc_nad);
    ...
    wrt_r_matrix(obs_lenc_mad, 2, 2);
    wrt_r_namevector(yrs_lenc_mad);
    wrt_r_namevector(lenbins);
    close_r_matrix();
...
open_r_df("t.series", styr, (endyr+1), 2);
    ...
    wrt_r_df_col("U.nad.ob", obs_cpue_nad);
    wrt_r_df_col("U.nad.pr", pred_nad_cpue);
    wrt_r_df_col("cv.U.nad", obs_cv_cpue_nad/w_cpue_nad);  //applied CV after weighting
    wrt_r_df_col("cv.unwgted.U.nad", obs_cv_cpue_nad); //CV before weighting

    wrt_r_df_col("U.mad.ob", obs_cpue_mad);
    wrt_r_df_col("U.mad.pr", pred_mad_cpue);
    wrt_r_df_col("cv.U.mad", obs_cv_cpue_mad/w_cpue_mad);  //applied CV after weighting
    wrt_r_df_col("cv.unwgted.U.mad", obs_cv_cpue_mad); //CV before weighting

    wrt_r_df_col("U.sad.ob", obs_cpue_sad);
    wrt_r_df_col("U.sad.pr", pred_sad_cpue);
    wrt_r_df_col("cv.U.sad", obs_cv_cpue_sad/w_cpue_sad);  //applied CV after weighting
    wrt_r_df_col("cv.unwgted.U.sad", obs_cv_cpue_sad); //CV before weighting

    wrt_r_df_col("U.jai.ob", obs_cpue_jai);
    wrt_r_df_col("U.jai.pr", pred_jai_cpue);
    wrt_r_df_col("cv.U.jai", obs_cv_cpue_jai/w_cpue_jai);  //applied CV after weighting
    wrt_r_df_col("cv.unwgted.U.jai", obs_cv_cpue_jai);  //CV before weighting
    
    //wrt_r_df_col("U.mareco.ob", obs_cpue_mareco);		 //wrt_r_df_col("acomp.cRn.nfish", nfish_cRn_agec_allyr);
    //wrt_r_df_col("U.mareco.pr", pred_mareco_cpue);
    //wrt_r_df_col("cv.U.mareco", obs_cv_cpue_mareco/w_cpue_mareco);  //applied CV after weighting
    //wrt_r_df_col("cv.unwgted.U.mareco", obs_cv_cpue_mareco);  //CV before weighting
    ...
    wrt_r_df_col("L.cRn.ob", obs_L_cRn);
    wrt_r_df_col("L.cRn.pr", pred_cRn_L_mt);
    wrt_r_df_col("cv.L.cRn", obs_cv_L_cRn);

    wrt_r_df_col("L.cRs.ob", obs_L_cRs);
    wrt_r_df_col("L.cRs.pr", pred_cRs_L_mt);
    wrt_r_df_col("cv.L.cRs", obs_cv_L_cRs);

    wrt_r_df_col("L.cBn.ob", obs_L_cBn);
    wrt_r_df_col("L.cBn.pr", pred_cBn_L_mt);
    wrt_r_df_col("cv.L.cBn", obs_cv_L_cBn);

    wrt_r_df_col("L.cBs.ob", obs_L_cBs);
    wrt_r_df_col("L.cBs.pr", pred_cBs_L_mt);
    wrt_r_df_col("cv.L.cBs", obs_cv_L_cBs);
...
open_r_df("U.series", 1, 26, 2);
    ...
    wrt_r_df_col("U.mareco.ob", obs_cpue_mareco);		 
    wrt_r_df_col("U.mareco.pr", pred_mareco_cpue);
    wrt_r_df_col("cv.U.mareco", obs_cv_cpue_mareco/w_cpue_mareco);  //applied CV after weighting
    wrt_r_df_col("cv.unwgted.U.mareco", obs_cv_cpue_mareco);  //CV before weighting
