...
cout << endl;
  ...
  nyr_lenc_pool_rGN_D.allocate("nyr_lenc_pool_rGN_D");
  yrs_lenc_pool_rGN_D.allocate(1,nyr_lenc_pool_rGN_D,"yrs_lenc_pool_rGN_D");
  nfish_lenc_pool_rGN_D.allocate(1,nyr_lenc_pool_rGN_D,"nfish_lenc_pool_rGN_D");
  ...
  set_selpar_A50_GRD.allocate(1,7,"set_selpar_A50_GRD");
  set_selpar_slope_GRD.allocate(1,7,"set_selpar_slope_GRD");
  set_selpar_A502_GRD.allocate(1,7,"set_selpar_A502_GRD");
  set_selpar_slope2_GRD.allocate(1,7,"set_selpar_slope2_GRD");
 ...
 model_data(argc,argv) , function_minimizer(sz)
{
  ...
  #endif
  pred_rGN_D_lenc_yr.allocate(1,nyr_lenc_pool_rGN_D,1,nlenbins,"pred_rGN_D_lenc_yr");
  #ifndef NO_AD_INITIALIZE
    pred_rGN_D_lenc_yr.initialize();
  #endif
  ...
  #endif
  neff_rGN_D_lenc_allyr.allocate(styr,endyr,"neff_rGN_D_lenc_allyr");
  #ifndef NO_AD_INITIALIZE
    neff_rGN_D_lenc_allyr.initialize();
  #endif
  ...
  #endif
  sel_rGN_D_vec.allocate(1,nages,"sel_rGN_D_vec");
  #ifndef NO_AD_INITIALIZE
    sel_rGN_D_vec.initialize();
  #endif
  selpar_A50_GRD.allocate(selpar_A50_GRD_LO,selpar_A50_GRD_HI,selpar_A50_GRD_PH,"selpar_A50_GRD");
  selpar_slope_GRD.allocate(selpar_slope_GRD_LO,selpar_slope_GRD_HI,selpar_slope_GRD_PH,"selpar_slope_GRD");
  selpar_A502_GRD.allocate(selpar_A502_GRD_LO,selpar_A502_GRD_HI,selpar_A502_GRD_PH,"selpar_A502_GRD");
  selpar_slope2_GRD.allocate(selpar_slope2_GRD_LO,selpar_slope2_GRD_HI,selpar_slope2_GRD_PH,"selpar_slope2_GRD");
  selpar_A50_GRD_out.allocate(1,8,"selpar_A50_GRD_out");
  #ifndef NO_AD_INITIALIZE
    selpar_A50_GRD_out.initialize();
  #endif
  selpar_slope_GRD_out.allocate(1,8,"selpar_slope_GRD_out");
  #ifndef NO_AD_INITIALIZE
    selpar_slope_GRD_out.initialize();
  #endif
  selpar_A502_GRD_out.allocate(1,8,"selpar_A502_GRD_out");
  #ifndef NO_AD_INITIALIZE
    selpar_A502_GRD_out.initialize();
  #endif
  selpar_slope2_GRD_out.allocate(1,8,"selpar_slope2_GRD_out");
  #ifndef NO_AD_INITIALIZE
    selpar_slope2_GRD_out.initialize();
  #endif
  prob_belowsizelim_block2rGN.allocate(1,nages,"prob_belowsizelim_block2rGN");
  #ifndef NO_AD_INITIALIZE
...
void model_parameters::get_length_comps(void)
{
  ...
  for (iyear=1;iyear<=nyr_lenc_pool_rGN_D;iyear++)
   {pred_rGN_D_lenc_yr(iyear)=(D_rGN_num(yrs_lenc_pool_rGN_D(iyear))*lenprob_rGN_D)/sum(D_rGN_num(yrs_lenc_pool_rGN_D(iyear)));}
  pred_rGN_D_lenc.initialize();
  for (iyear=1;iyear<=nyr_lenc_pool_rGN_D;iyear++)
    {pred_rGN_D_lenc(1) += nfish_lenc_pool_rGN_D(iyear) * pred_rGN_D_lenc_yr(iyear);}
  pred_rGN_D_lenc(1)=pred_rGN_D_lenc(1)/sum(nfish_lenc_pool_rGN_D);
 //general rec   
  //for (iyear=1;iyear<=nyr_rGN_lenc;iyear++) 
