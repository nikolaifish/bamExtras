...
open_r_info_list("info", true);
	wrt_r_item("title", "SEDAR 68 Assessment");
...
open_r_df("parm.cons",1,8,2);
	...
	wrt_r_df_col("log.dm.lenc.sCT",log_dm_sCT_lc_out);
	wrt_r_df_col("log.dm.agec.cGN",log_dm_cGN_ac_out);
    ...
    wrt_r_df_col("selpar.A50.cGN3",selpar_A50_cGN3_out);
    wrt_r_df_col("selpar.slope.cGN3",selpar_slope_cGN3_out);

    wrt_r_df_col("selpar.A50.rGN1",selpar_A50_rGN1_out);
    wrt_r_df_col("selpar.slope.rGN1",selpar_slope_rGN1_out);
    wrt_r_df_col("selpar.A501.rGN3",selpar_A50_rGN3_out);
    wrt_r_df_col("selpar.slope1.rGN3",selpar_slope_rGN3_out);
	...
	//wrt_r_df_col("selpar.A501.sVD",selpar_A50_sVD_out);
	//wrt_r_df_col("selpar.slope1.sVD",selpar_slope_sVD_out);
	//wrt_r_df_col("selpar.A502.sVD",selpar_A502_sVD_out);
	//wrt_r_df_col("selpar.slope2.sVD",selpar_slope2_sVD_out);


	wrt_r_df_col("log.q.cpue.cGN",log_q_cGN_out);
    wrt_r_df_col("log.q.cpue.rGN",log_q_rGN_out);
    wrt_r_df_col("log.q.cpue.sCT",log_q_sCT_out);
	//wrt_r_df_col("log.q.sVD",log_q_sVD_out);
...
open_r_df("parm.tvec", styr, endyr, 2);
	...
	wrt_r_df_col("log.q.dev.cGN.RWq", q_RW_log_dev_cGN);
...
open_r_vector("like");
    ...
    wrt_r_item("lk.lenc.cGN", f_cGN_lenc);
		...
		//agecomps
		wrt_r_item("lk.agec.cGN", f_cGN_agec);
...
open_r_list("comp.mats");
    ...
    wrt_r_matrix(obs_lenc_sCT, 2, 2);
    wrt_r_namevector(yrs_lenc_sCT);
    wrt_r_namevector(lenbins);
    close_r_matrix();
    ...
    open_r_matrix("acomp.cGN.ob");
    wrt_r_matrix(obs_agec_cGN, 2, 2);
    wrt_r_namevector(yrs_agec_cGN);
    wrt_r_namevector(agebins_agec);
    close_r_matrix();

    open_r_matrix("acomp.cGN.pr");
    wrt_r_matrix(pred_cGN_agec, 2, 2);
    wrt_r_namevector(yrs_agec_cGN);
    wrt_r_namevector(agebins_agec);
    close_r_matrix();
...
// DATA FRAME of time series
    ...
    wrt_r_df_col("U.cGN.ob", obs_cpue_cGN);
    wrt_r_df_col("U.cGN.pr", pred_cGN_cpue);
    wrt_r_df_col("cv.U.cGN", obs_cv_cpue_cGN/w_cpue_cGN);  //applied CV after weighting
    wrt_r_df_col("cv.unwgted.U.cGN", obs_cv_cpue_cGN); //CV before weighting
    ...
    wrt_r_df_col("U.sCT.pr", pred_sCT_cpue);
   	wrt_r_df_col("cv.U.sCT", obs_cv_cpue_sCT/w_cpue_sCT);
		...
		//wrt_r_df_col("U.sVD.ob", obs_sVD_cpue);
		//wrt_r_df_col("U.sVD.pr", pred_sVD_cpue);
		//wrt_r_df_col("cv.U.sVD", sVD_cpue_cv/w_I_sVD);
		//wrt_r_df_col("cv.unwgted.U.sVD", sVD_cpue_cv);
    ...
    wrt_r_df_col("acomp.cGN.n", nsamp_cGN_agec_allyr);
    ...
    wrt_r_df_col("acomp.cGN.nfish", nfish_cGN_agec_allyr);
    ...
    wrt_r_df_col("acomp.cGN.neff", (1+nsamp_cGN_agec_allyr*exp(log_dm_cGN_ac_out(8)))/(1+exp(log_dm_cGN_ac_out(8))) );
...
close_r_df();
 ...
 open_r_df("a.series", 1, nages, 2);
	...
	wrt_r_df_col("length.L", meanlen_TL_L);
 	wrt_r_df_col("length.L.cv", len_cv_L);
 	wrt_r_df_col("length.L.sd", len_sd_L);
