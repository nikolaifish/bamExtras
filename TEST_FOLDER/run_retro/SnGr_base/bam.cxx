...
open_r_info_list("info", true);
	wrt_r_item("title", "SEDAR 36 Update Assessment");
	wrt_r_item("species", "SA Snowy Grouper");
...
open_r_info_list("parms", false);
	...
	wrt_r_item("q.sVL", mfexp(log_q_cpue_sVL));
...
open_r_df("parm.cons",1,8,2);
	...
	//wrt_r_df_col("log.dm.rHB.lc",log_dm_rHB_lc_out);
	...
	wrt_r_df_col("log.dm.agec.cLL",log_dm_cLL_ac_out);
	...
	wrt_r_df_col("log.dm.agec.sVL",log_dm_sVL_ac_out);
    ...
    // wrt_r_df_col("selpar.slope1.cLL2",selpar_slope_cLL2_out);
	...
	wrt_r_df_col("selpar.afull.rGN1",selpar_afull_rGN1_out);
	wrt_r_df_col("selpar.sigma.rGN1",selpar_sigma_rGN1_out);
 	wrt_r_df_col("selpar.A501.rGN2",selpar_A50_rGN2_out);
    wrt_r_df_col("selpar.slope1.rGN2",selpar_slope_rGN2_out);
	wrt_r_df_col("selpar.afull.rGN2",selpar_afull_rGN2_out);
	wrt_r_df_col("selpar.sigma.rGN2",selpar_sigma_rGN2_out);
	wrt_r_df_col("selpar.A50.rGN3",selpar_A50_rGN3_out);
    wrt_r_df_col("selpar.slope.rGN3",selpar_slope_rGN3_out);
	wrt_r_df_col("selpar.afull.rGN3",selpar_afull_rGN3_out);
	wrt_r_df_col("selpar.sigma.rGN3",selpar_sigma_rGN3_out);
	...
	wrt_r_df_col("selpar.afull.sCT",selpar_afull_sCT_out);
	wrt_r_df_col("selpar.sigma.sCT",selpar_sigma_sCT_out);
	
	wrt_r_df_col("selpar.A501.sVL",selpar_A50_sVL_out);
    wrt_r_df_col("selpar.slope1.sVL",selpar_slope_sVL_out);
	...
	wrt_r_df_col("log.q.cpue.sVL",log_q_sVL_out);
    ...
    //wrt_r_df_col("log.avg.F.rHB",log_avg_F_rHB_out);
    wrt_r_df_col("log.avg.F.L.rGN",log_avg_F_rGN_out);
    //wrt_r_df_col("log.avg.F.cHL.D",log_avg_F_cHL_D_out);
    //wrt_r_df_col("log.avg.F.rHB.D",log_avg_F_rHB_D_out);
    //wrt_r_df_col("log.avg.F.rGN.D",log_avg_F_rGN_D_out);
    //wrt_r_df_col("F.init",F_init_out);
close_r_df();
...
open_r_df("parm.tvec.cons",1,3,2);
    ...
    //wrt_r_df_col("log.F.dev.cHL.D",set_log_F_dev_cHL_D);
    //wrt_r_df_col("log.F.dev.rHB.D",set_log_F_dev_rHB_D);
    //wrt_r_df_col("log.F.dev.rGN.D",set_log_F_dev_rGN_D);
...
open_r_vector("sdnr");
	...
	wrt_r_item("sdnr.U.sVL", sdnr_I_sVL);
	...
	wrt_r_item("sdnr.ac.sVL", sdnr_ac_sVL);
...
open_r_vector("like");
	...
	wrt_r_item("lk.U.sVL", f_sVL_cpue);
	...
	wrt_r_item("lk.U.rGN.RWq", f_rGN_RWq_cpue);
    wrt_r_item("lk.priors",f_priors);
    wrt_r_item("gradient.max",grad_max);

    wrt_r_item("w.L", w_L);
    //wrt_r_item("w.D", w_D);
    //wrt_r_item("w.U.cHL", w_I_cHL);
    //wrt_r_item("w.U.rHB", w_I_rHB);
    //wrt_r_item("w.U.rGN", w_cpue_rGN);
	wrt_r_item("w.U.sCT", w_cpue_sCT);
	wrt_r_item("w.U.sVL", w_cpue_sVL);
	...
	wrt_r_item("w.ac.sVL", w_agec_sVL);
...
open_r_list("sel.age");
	...
	open_r_matrix("sel.m.sVL");
    wrt_r_matrix(sel_sVL, 2, 2);
    wrt_r_namevector(styr,endyr);
    wrt_r_namevector(agebins);
    close_r_matrix();
...
open_r_list("comp.mats");
    ...
    // wrt_r_matrix(obs_rHB_D_lenc, 2, 2);
    // wrt_r_namevector(yrs_rHB_D_lenc);
    // wrt_r_namevector(lenbins);
    // close_r_matrix();
	...
	open_r_matrix("acomp.cLL.ob");
    wrt_r_matrix(obs_agec_cLL, 2, 2);
    wrt_r_namevector(yrs_agec_cLL);
    wrt_r_namevector(agebins_agec);
    close_r_matrix();

    open_r_matrix("acomp.cLL.pr");
    wrt_r_matrix(pred_cLL_agec, 2, 2);
    wrt_r_namevector(yrs_agec_cLL);
    wrt_r_namevector(agebins_agec);
    close_r_matrix();
	...
	open_r_matrix("acomp.sVL.ob");
    wrt_r_matrix(obs_agec_sVL, 2, 2);
    wrt_r_namevector(yrs_agec_sVL);
    wrt_r_namevector(agebins_agec);
    close_r_matrix();

    open_r_matrix("acomp.sVL.pr");
    wrt_r_matrix(pred_sVL_agec, 2, 2);
    wrt_r_namevector(yrs_agec_sVL);
    wrt_r_namevector(agebins_agec);
    close_r_matrix();
...
open_r_df("t.series", styr, (endyr+1), 2);
    ...
    // wrt_r_df_col("U.cHL.ob", obs_cHL_cpue);
    // wrt_r_df_col("U.cHL.pr", pred_cHL_cpue);
    // wrt_r_df_col("cv.U.cHL", cHL_cpue_cv/w_I_cHL);  //applied CV after weighting
    // wrt_r_df_col("cv.unwgted.U.cHL", cHL_cpue_cv); //CV before weighting

    // wrt_r_df_col("U.rHB.ob", obs_rHB_cpue);
    // wrt_r_df_col("U.rHB.pr", pred_rHB_cpue);
    // wrt_r_df_col("cv.U.rHB", rHB_cpue_cv/w_I_rHB);
	// wrt_r_df_col("cv.unwgted.U.rHB", rHB_cpue_cv);
	...
	wrt_r_df_col("U.sVL.ob", obs_cpue_sVL);
    wrt_r_df_col("U.sVL.pr", pred_sVL_cpue);
   	wrt_r_df_col("cv.U.sVL", obs_cv_cpue_sVL/w_cpue_sVL);
	wrt_r_df_col("cv.unwgted.U.sVL", obs_cv_cpue_sVL);
	...
	wrt_r_df_col("q.sVL", q_sVL);
    wrt_r_df_col("q.sVL.RW.log.dev",q_RW_log_dev_sVL);
    ...
    //wrt_r_df_col("cv.D.cHL", cHL_D_cv);
    ...
    // wrt_r_df_col("cv.D.rGN", rGN_D_cv);
	...
	wrt_r_df_col("acomp.cLL.n", nsamp_cLL_agec_allyr);
	...
	wrt_r_df_col("acomp.cLL.nfish", nfish_cLL_agec_allyr);
	...
	wrt_r_df_col("acomp.sVL.nfish", nfish_sVL_agec_allyr);
	...
	wrt_r_df_col("acomp.cLL.neff", (1+nsamp_cLL_agec_allyr*exp(log_dm_cLL_ac_out(8)))/(1+exp(log_dm_cLL_ac_out(8))) );
    ...
    wrt_r_df_col("acomp.sVL.neff", (1+nsamp_sVL_agec_allyr*exp(log_dm_sVL_ac_out(8)))/(1+exp(log_dm_sVL_ac_out(8))) );  	
